...
exports.obbIntersect = function (t, e, i, o, n, s) {
    ...
    var ot = (F >= 0 ? F : -F) + Q;
    return !(
        (W >= 0 ? W : -W) > a + (v * Z + b * X + w * Y) ||
        (q >= 0 ? q : -q) > l + (v * J + b * $ + w * tt) ||
        (K >= 0 ? K : -K) > c + (v * et + b * it + w * ot) ||
        ((r = W * E + q * D + K * U) >= 0 ? r : -r) > a * Z + l * J + c * et + v ||
        ((r = W * M + q * G + K * j) >= 0 ? r : -r) > a * X + l * $ + c * it + b ||
        ((r = W * T + q * N + K * F) >= 0 ? r : -r) > a * Y + l * tt + c * ot + w ||
        ((r = K * D - q * U) >= 0 ? r : -r) > l * et + c * J + b * Y + w * X ||
        ((r = K * G - q * j) >= 0 ? r : -r) > l * it + c * $ + v * Y + w * Z ||
        ((r = K * N - q * F) >= 0 ? r : -r) > l * ot + c * tt + v * X + b * Z ||
        ((r = W * U - K * E) >= 0 ? r : -r) > a * et + c * Z + b * tt + w * $ ||
        ((r = W * j - K * M) >= 0 ? r : -r) > a * it + c * X + v * tt + w * J ||
        ((r = W * F - K * T) >= 0 ? r : -r) > a * ot + c * Y + v * $ + b * J ||
        ((r = q * E - W * D) >= 0 ? r : -r) > a * J + l * Z + b * ot + w * it ||
        ((r = q * M - W * G) >= 0 ? r : -r) > a * $ + l * X + v * ot + w * et ||
        ((r = q * T - W * N) >= 0 ? r : -r) > a * tt + l * Y + v * it + b * et
    );
};
var d = new cc.Vec2();
var p = new cc.Vec2();
